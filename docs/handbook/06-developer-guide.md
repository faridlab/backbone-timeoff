...
metaphor migration generate Invoice timeoff
metaphor migration run
...
(`timeoff` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed timeoff          # run Rust seeders
metaphor migration generate-seeds timeoff  # emit SQL seed files
```
