...
//!   payload (company, request, employee, window)
...
async fn seed_balance(pool: &PgPool, company_id: Uuid, type_id: Uuid, employee_id: Uuid) {
    sqlx::query(
        r#"INSERT INTO timeoff.timeoff_balances
               (id, company_id, timeoff_type_id, employee_id, period, allocated, used)
           VALUES ($1, $2, $3, $4, '2026', 30, 0)"#,
    )
    .bind(Uuid::new_v4())
    .bind(company_id)
    .bind(type_id)
...
async fn to_sink_emits_on_approve_refuse_cancel_void() {
    ...
    seed_balance(&pool, company, type_id, employee).await;
    ...
    let req = svc
        .submit_request(company, type_id, employee, day(2026, 5, 4), day(2026, 5, 5), None)
        .await
        .unwrap();
    ...
    let req = svc
        .submit_request(company, type_id, employee, day(2026, 7, 10), day(2026, 7, 9), None)
        .await
        .unwrap();
    svc.approve_request(req, None).await.unwrap();
    let events = sink.events();
...
async fn to_default_sink_keeps_verb_behavior() {
    ...
    let (used,): (Decimal,) = sqlx::query_as(
        "SELECT used FROM timeoff.timeoff_balances \
         WHERE company_id = $1 AND timeoff_type_id = $2 AND employee_id = $3 AND period = '2026'",
    )
    .bind(company)
    .bind(type_id)
    .bind(employee)
